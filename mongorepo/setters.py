...
def _set_array_fields_methods(cls: type, integer_fields: Iterable[str]) -> None:
    ...
    for field in integer_fields:
        ...
        if type(dto_fields[field]) is not list:
...
def _set_integer_fields_methods(cls: type, array_fields: Iterable[str]) -> None:
    attributes = _get_collection_and_dto(cls)
    dto, collection = attributes['dto'], attributes['collection']
    dto_fields = get_default_values(dto)
